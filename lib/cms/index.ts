...
import { CmsQueryError, type CmsAuthor, type CmsPost, type CmsTagCounts } from './types'
...
function normalizePost(value: Record<string, unknown>): SitePost {
  const slugValue = String(value.slug || '')
...
export const getAllPosts = cache(async (): Promise<CmsPost[]> => {
  ...
  try {
    ...
    withCmsErrorContext(
      '[cms] failed to load posts',
      'getAllPosts',
      error,
      env.PAYLOAD_POSTS_COLLECTION
    )
  }
})

export async function getHomePosts(limit = 5): Promise<CmsPost[]> {
  const posts = await getAllPosts()
  return posts.slice(0, limit)
}

export async function getPublishedPostsByTag(tagSlug: string): Promise<CmsPost[]> {
  const posts = await getAllPosts()
...
export const getPostBySlug = cache(async (slugValue: string): Promise<CmsPost | null> => {
  ...
  try {
    ...
    withCmsErrorContext(
      '[cms] failed to load post by slug',
      'getPostBySlug',
      error,
      env.PAYLOAD_POSTS_COLLECTION
    )
...
export async function getTagCounts(): Promise<CmsTagCounts> {
  const posts = await getAllPosts()
...
export type { CmsPost, CmsAuthor, CmsTagCounts }
