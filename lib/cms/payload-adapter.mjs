...
async function resolvePayloadConfig() {
  ...
  if (explicitConfigPath) {
    const explicitConfig = await tryImport(explicitConfigPath)
  ...
  for (const specifier of staticCandidates) {
    const config = await tryImport(specifier)
    if (config) return config
  }

  await ensureTsLoaderRegistered()

  const tsCandidates = ['../../payload.config.ts', '../../src/payload.config.ts']
...
async function fetchCollectionWithRest(collection) {
  ...
  const requestCookie = await getRequestCookieHeader()
  ...
  while (true) {
    ...
    if (requestCookie) {
      requestHeaders.cookie = requestCookie
    }
