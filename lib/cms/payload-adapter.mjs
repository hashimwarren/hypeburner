...
let hasLoggedPayloadInitError = false
let tsLoaderRegistrationPromise
...
function tryRequire(specifier) {
  try {
    return unwrapModule(requireFromCwd(specifier))
  } catch {
    return null
  }
}

async function ensureTsLoaderRegistered() {
  if (tsLoaderRegistrationPromise) return tsLoaderRegistrationPromise

  tsLoaderRegistrationPromise = (async () => {
    const registerModule = await tryImport('esbuild-register/dist/node')
    if (!registerModule) return false
    const register = registerModule.register || registerModule.default?.register
  ...
  return tsLoaderRegistrationPromise
}
...
async function resolvePayloadConfig() {
  ...
  const explicitConfigPath = process.env.PAYLOAD_CONFIG_PATH?.trim()

  if (explicitConfigPath) {
    const explicitPath = path.isAbsolute(explicitConfigPath)
      ? explicitConfigPath
      : path.join(cwd, explicitConfigPath)
    const explicitConfig =
      (await tryImport(pathToFileURL(explicitPath).href)) || tryRequire(explicitPath)
    if (explicitConfig) return explicitConfig
  }

  const staticCandidates = [
    '@payload-config',
    path.join(cwd, 'payload.config.js'),
    path.join(cwd, 'payload.config.mjs'),
    path.join(cwd, 'src', 'payload.config.js'),
    path.join(cwd, 'src', 'payload.config.mjs'),
  ]

  for (const specifier of staticCandidates) {
    const config =
      (specifier.startsWith('@') ? await tryImport(specifier) : null) ||
      (specifier.startsWith('@') ? tryRequire(specifier) : null) ||
      (specifier.startsWith('@') ? null : await tryImport(pathToFileURL(specifier).href)) ||
      (specifier.startsWith('@') ? null : tryRequire(specifier))
    if (config) return config
  }

  await ensureTsLoaderRegistered()

  const tsCandidates = [path.join(cwd, 'payload.config.ts'), path.join(cwd, 'src', 'payload.config.ts')]

  for (const specifier of tsCandidates) {
    const config = (await tryImport(pathToFileURL(specifier).href)) || tryRequire(specifier)
    if (config) return config
  }
...
async function getPayloadClient() {
  ...
  payloadClientPromise = (async () => {
    try {
      const payloadModule = await tryImport('payload')
      const getPayload = payloadModule?.getPayload
      if (typeof getPayload !== 'function') return null
      ...
      if (!hasLoggedPayloadInitError) {
        hasLoggedPayloadInitError = true
        console.warn('[payload-adapter] Local Payload API unavailable; falling back to REST API', error)
      }
      return null
    }
  })().then((client) => {
...
async function getRequestAuthHeaders() {
  const headersModule = await tryImport('next/headers')
  if (!headersModule?.headers) return {}
...
async function fetchCollectionWithRest(collection) {
  ...
  while (true) {
    ...
    for (const [key, value] of Object.entries(requestAuthHeaders)) {
